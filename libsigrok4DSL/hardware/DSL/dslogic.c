...
static int dev_acquisition_start(struct sr_dev_inst *sdi, void *cb_data)
{
	...
	//////////////////////////////////////////////////////////////////
	// @Noema
	// 来自IDA逆向官方版本DSView.exe v1.3.1，通过错误日志字符串定位此处位置
	// 进而得到官方版本在Windows下如何绕过Windows下不支持libusb_get_pollfds的解决方法
	// 同时在dsl.c中的dsl_dev_open函数同步做调整
	//
    /* setup callback function for data transfer */
	/*
    lupfd = libusb_get_pollfds(drvc->sr_ctx->libusb_ctx);
	if(!lupfd) {
		sr_err("%s,ERROR: libusb_get_pollfds failed, return 0x%x.", __func__, lupfd);
		return SR_ERR;
	}
    for (i = 0; lupfd && lupfd[i]; i++);
	...
	*/
	...
	timeout = dsl_get_timeout(sdi);
	sr_err("%s: timeout == %d.", __func__, timeout);
	
	if(!devc->channel) {
		sr_err("%s: devc->channel == NULL.", __func__);
		return SR_ERR;
	}
	
	sr_session_source_add_channel(devc->channel, 9, timeout, receive_data, sdi);
